...
app.config['MYSQL_USER'] = '3oVRZToUYY'
app.config['MYSQL_PASSWORD'] = 'Ek5cYkpYJz'
app.config['MYSQL_DB'] = '3oVRZToUYY'
...
def login():
    ...
    if(request.method=="POST"):
        ...
        cursor1.execute('SELECT * FROM users WHERE email= %s AND pass1= %s', [str(email),str(password)])
        ...
        if(email_info==None):
            ...
            cursor4.execute('SELECT sum(exp) FROM exp  WHERE email = %s',[session['email']])
